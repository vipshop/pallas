...
function callES(server, url, method, data, successCallback, completeCallback, request, options) {
	if(url.indexOf("/pallas/") < 0){
		url = "/pallas/esproxy/"+url;
	}
    ...
    $.ajax({
        ...
        beforeSend:function(xhr){
          ...
          xhr.setRequestHeader('es-host', currentHost);
          xhr.setRequestHeader('es-cluster', getSearchUrlVar("cluster"));
        },
        password: password,
        username: uname,
        crossDomain: true,
		...
		error:function(){
			...
			callES(server, url, method, data, successCallback, completeCallback, request, options);
		}
    });
}

function stringedBigIntJSON(origJSON) {
    ...
    callES(es_server, es_url, es_method, es_data, null, function (xhr, status) {
            ...
            if (typeof xhr.status == "number" &&
                ...
                var value = xhr.responseText,
                    ...
                    firstLine = value.substring(0, jsonStart),
                    jsonStr = value.substring(jsonStart),
                    json;
                try {
                    json = stringedBigIntJSON(jsonStr);
                ...
                if (json.hits && json.hits.hits) {
                    sense.output.getSession().setCsv(ConvertSourceToCSV(json.hits.hits));
                }
            }
            else {
...
function querySave() {
    ...
    var es_server = getSearchUrlVar("host"),
        es_url = req.url,
        es_method = req.method,
        es_data = req.data;
...
function init() {
    ...
    es_server.val(getSearchUrlVar("host"));
