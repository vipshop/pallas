...
<script>
import TargetGroupItem from './target_group_item';
import TargetGroupInfoDialog from './target_group_info_dialog';
...
export default {
  ...
  methods: {
    ...
    getTargetGroupTree() {
      ...
      this.loading = true;
      this.$http.post('/index/routing/target_group/list.json', params).then((data) => {
        this.targetGroupTree = data.map((obj) => {
          const rObj = {};
        ...
        this.targetGroupTreeNoChildren = data.map((obj) => {
          const rObj = {};
          rObj.cluster = obj.cluster;
          rObj.address = obj.address;
          rObj.name = obj.name;
