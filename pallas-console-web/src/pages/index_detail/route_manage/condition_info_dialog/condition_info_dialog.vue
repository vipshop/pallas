            ...
            <el-row :gutter="30">
                <el-col :span="10">
                    ...
                    <el-form-item label="Preference" prop="preference">
                        <el-select v-model="conditionInfo.preference" filterable allow-create>
                            <el-option label="无" value=""></el-option>
                            <el-option v-for="item in preferenceList" :label="item" :value="item" :key="item"></el-option>
                        </el-select>
                        <span class="warning">
                            <el-tooltip class="item" effect="dark" placement="right">
                                <div slot="content">具体参考Elasticsearch关于Preference相关文档
                                    <br/>示例：
                                    <br/>_primary：只选择primary分片查询
                                    <br/>_primary_first：优先选择primary分片查询
                                    <br/>_replica：只选择replica分片查询
                                    <br/>_replica_first：优先选择replica分片查询
                                    <br/>_only_local：只查询本地节点
                                    <br/>_local：优先查询本地节点
                                    <br/>_prefer_nodes:abc,xyz：优先查询节点id为abc和xyz的节点
                                    <br/>_shards:2,3：只查询第2，3组分片
                                </div>
...
export default {
  ...
  methods: {
    validatePreference(rule, value, callback) {
