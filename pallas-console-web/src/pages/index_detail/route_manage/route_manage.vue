<template>
    <div class="my-tab-content" v-loading="loading" element-loading-text="请稍等···">
        <div class="content" v-if="privilege">
        ...
        <div class="content">
            <el-table :data="rulesList" stripe style="width: 100%">
                ...
                <el-table-column label="规则关系" prop="conditionRelation" width="80px" header-align="center"></el-table-column>
                ...
                <el-table-column label="Preference" min-width="70" header-align="center">
                    <template scope="scope">
                        <el-tag class="target-group-item" :type="scope.row.preference === '' ? 'info' : 'success'" close-transition>
                            <span>{{scope.row.preference || '无'}}</span>
                        </el-tag>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="80px" header-align="center" v-if="privilege">
                    <template scope="scope">
        ...
        <target-group :privilege="privilege" :index-id="indexId" :index-name="indexName" :rules-list="rulesList" :routing-target-groups-list="routingTargetGroupsList" @get-routing-list="getRoutingList"></target-group>
...
<script>
import TargetGroup from './target_group/target_group';
import ConditionInfoDialog from './condition_info_dialog/condition_info_dialog';
...
export default {
  ...
  methods: {
    ...
    updateConditionRequest(rulesList) {
      ...
      this.loading = true;
      this.$http.post('/index/routing/rule/update.json', params).then(() => {
    ...
    getRoutingList() {
      ...
      this.loading = true;
      this.$http.post('/index/routing/list.json', params).then((data) => {
