...
angular.module('cerebro', ['ngRoute', 'ngAnimate', 'ui.bootstrap'])
  .config(['$routeProvider',
    function($routeProvider) {
      $routeProvider
        .when('/overview', {
          templateUrl: 'tpl/overview.html'+version,
          controller: 'OverviewController'
        })
        .when('/nodes', {
          templateUrl: 'tpl/nodes/index.html'+version,
          controller: 'NodesController'
        })
        .when('/connect', {
          templateUrl: 'tpl/connect.html'+version,
          controller: 'ConnectController'
        })
        .when('/rest', {
          templateUrl: 'tpl/rest/index.html'+version,
          controller: 'RestController'
        })
        .when('/aliases', {
          templateUrl: 'tpl/aliases.html'+version,
          controller: 'AliasesController'
        })
        .when('/create', {
          templateUrl: 'tpl/create_index.html'+version,
          controller: 'CreateIndexController'
        })
        .when('/analysis', {
          templateUrl: 'tpl/analysis/index.html'+version,
          controller: 'AnalysisController'
        })
        .when('/templates', {
          templateUrl: 'tpl/templates/index.html'+version,
          controller: 'TemplatesController'
        })
        .when('/cluster_settings', {
          templateUrl: 'tpl/cluster_settings/index.html'+version,
          controller: 'ClusterSettingsController'
        })
        .when('/index_settings', {
          templateUrl: 'tpl/index_settings/index.html'+version,
          controller: 'IndexSettingsController'
        })
        .when('/snapshot', {
          templateUrl: 'tpl/snapshot/index.html'+version,
          controller: 'SnapshotController'
        })
        .when('/repository', {
          templateUrl: 'tpl/repositories/index.html'+version,
          controller: 'RepositoriesController'
        })
        .when('/cat', {
          templateUrl: 'tpl/cat/index.html'+version,
          controller: 'CatController'
        })
        .when('/dashboard', {
          templateUrl: 'tpl/dashboard/index.html'+version,
...
angular.module('cerebro').directive('analysisTokens', function() {
  return {
    ...
    templateUrl: 'tpl/analysis/tokens.html'
  };
});
...
angular.module('cerebro').directive('clusterSetting', function() {
  return {
    ...
    templateUrl: 'tpl/cluster_settings/cluster_setting.html'
  };
});
...
angular.module('cerebro').controller('NavbarController', ['$scope', '$http',
  'PageService', 'DataService', 'RefreshService','$location','$rootScope','$timeout',
  function($scope, $http, PageService, DataService, RefreshService,$location,$rootScope,$timeout) {
    ...
    $scope.cluster = $location.search().cluster;
    ...
    var locationChangeSuccess = $rootScope.$on('$routeChangeSuccess', function(){
  ...
  function($scope, $http, $window, $location, OverviewDataService, AlertService,
    ...
    $scope.setNodes = function(nodes) {
      var ok = function(data) {
      	var excludeIps = '';
      	if (data.transient) {
      		excludeIps = data.transient['cluster.routing.allocation.exclude._ip']||'';
        }
      	$scope.nodes = nodes.filter(function(node) {
          var excluded = excludeIps.split(',').indexOf(node.ip) != -1;
      		node._excluded = excluded;
              	return $scope.nodes_filter.matches(node);
        });
      }
      DataService.send('/pallas/esproxy/_cluster/settings?flat_settings=true',{}, ok, error);
    };
    ...
    $scope.excludeNode = function(node, exclude) {
      ModalService.promptConfirmation(
        (exclude?'Exclude':'Include') + ' this node ' + node + '?',
        function() {
          var afterAll = function(data) {
            RefreshService.refresh();
            AlertService.success(data.result, data);
          };
          var updateRoutingFun = function() {
          	AlertService.info('Exclude node succeed, now going to update pallas search routing.', {});
          	OverviewDataService.updateRouting({cluster:$location.search().cluster}, afterAll, error);
          }
          OverviewDataService.excludeNode({nodeIp: node, exclude:exclude, cluster:$location.search().cluster}, updateRoutingFun, error);
        }
      );
    };
...
angular.module('cerebro').factory('OverviewDataService', ['DataService',
  function(DataService) {
    ...
    this.excludeNode = function(data, success, error) {
      DataService.send('/pallas/exclude/node.json', data, success, error);
    };

    this.updateRouting = function(data, success, error) {
      DataService.send('/pallas/ss/update_routing.json', data, success, error);
    };
...
angular.module('cerebro').controller('DashboardController', ['$scope', '$location','$http',
  '$sce', 'RestDataService', 'AlertService', 'ModalService', 'AceEditorService',
  'ClipboardService',
  function($scope, $location, $http, $sce, RestDataService, AlertService, ModalService,
           AceEditorService, ClipboardService) {
    $scope.host = RestDataService.getHost();
    $scope.bigdeskUrl = $sce.trustAsResourceUrl("/bigdesk/?cluster="+$location.search().cluster+"&endpoint="+$scope.host+"&connect=true#nodes");

    if(!window.dashboardInterval){
      window.dashboardInterval = setInterval(function(){

        var iframe = $(".dashboard iframe");         
        try {               
...
angular.module('cerebro').controller('RestController', ['$scope', '$location', '$http',
  '$sce', 'RestDataService', 'AlertService', 'ModalService', 'AceEditorService',
  'ClipboardService',
  function($scope, $location, $http, $sce, RestDataService, AlertService, ModalService,
           AceEditorService, ClipboardService) {
    ...
    $scope.senseUrl = $sce.trustAsResourceUrl("/sense/?cluster="+cluster+"&host="+$scope.host);
...
angular.module('cerebro').factory('RestDataService', ['DataService',
  function(DataService) {
    ...
    this.history = function(success, error) {
      DataService.send('rest/history.json', {}, success, error);
    };

    this.execute = function(method, path, data, success, error) {
      var requestData = {method: method, data: data, path: path};
      DataService.send('rest/request.json', requestData, success, error);
    };
...
angular.module('cerebro').directive('ngProgress',
  function() {
    ...
    return {
      ...
      templateUrl: 'tpl/shared/progress.html'
    };
  }
);
...
angular.module('cerebro').filter('trimUselessInfo', function() {
  return function(name) {
    if (name.length <= 18) {
      return name;
    }
    return name.replace(".idc.vip.com","").replace(".IDC.VIP.COM","").replace(".vip.vip.com","").replace(".VIP.VIP.COM","");
  };
});
...
angular.module('cerebro').factory('DataService', ['$rootScope', '$timeout',
  ...
  function($rootScope, $timeout, $http, $location, RefreshService,
    ...
    if ($location.search().cluster) {
      cluster = $location.search().cluster;
    }
    ...
    var clusterRequest = function(path, data, success, error) {
      if (host) {
        ...
        var config = {
          method: path.indexOf('esproxy/') >=0?'GET':'POST',
          url: path,
    ...
    var request = function(config, success, error) {
  	  if (config.url.indexOf('.html') < 0 && config.url.indexOf('/pallas/') < 0) {
        if (config.url.indexOf('exclude/node') >=0 || config.url.indexOf('ss/update_routing') >=0 || config.url.indexOf('esproxy/') >=0 ) {
			    config.url = config.url;	
		    } else if (config.url.indexOf('.json') < 0) {
			    config.url = "/pallas/cerebro/"+config.url;
        } else { 
			    config.url = "/pallas/cerebro_proxy/"+config.url;        
        }
  	  }
	  
	  var currentHost = $location.search().host;
	  var hosts = $location.search().host;
      ...
      if(!currentHost.includes("http")) {
        currentHost = "http://" + currentHost;
      }
    }


      config.headers = {"es-cluster":$location.search().cluster, "es-host":currentHost, "es-request":true};
	
	  if(config.data && config.data.host){
		  config.data.host = currentHost;
	  }
	  
      var handleSuccess = function(data) {
        if (data.status === 303) {
          top.location.href = "/pallas/login";
        } else {
          if (data == '') {
            success('ok');
          } else if (data.status == 0 || (data.status >= 200 && data.status < 300)) {
          	if (data.body) {
            	success(data.body);
            } else if (data.data) {
            	success(data.data);
      ...
      var handleError = function(data) {
        ...
        if(config.retryCount >= hostList.length) {
...
angular.module('cerebro').factory('RefreshService',
  function($location, $timeout) {
    ...
    var interval, timer;
    if (localStorage) {
      var key = $location.search().cluster + "_refreshInterval";
      interval = localStorage.getItem(key) || 5000;
    } else {
      interval = 5000;
    }
    ...
    this.setInterval = function(newInterval) {
      ...
      if (localStorage) {
        var key = $location.search().cluster + "_refreshInterval";
        localStorage.setItem(key, newInterval);
      }
    ...
    var autoRefresh = function(instance) {
      instance.refresh();
      timer = $timeout(function() { autoRefresh(instance); }, interval);
    };
    ...
    var init = function(){
      if(!window.refreshInterval){
        window.refreshInterval = setInterval(function(){
          ...
          var iframe = $(".cluster_dashboard",parent.document);         
          try {               
              var bHeight = $(".main_content_box").height() + 50; 
              if(Math.abs(iframe.height() - bHeight  ) > 4) {
