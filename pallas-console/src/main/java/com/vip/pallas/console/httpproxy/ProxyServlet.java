...
public class ProxyServlet extends HttpServlet {
  ...
  protected HttpClient createHttpClient(final RequestConfig requestConfig) {
    return HttpClientBuilder.create().setMaxConnPerRoute(50)
  ...
  public static void main(String[] args) {
	        ...
	        //map和flatmap的区别  
	        list.stream().map(item -> Arrays.stream(item.split(" "))).distinct().forEach(System.out::println);  
	        System.out.println("---------- ");  
  ...
  protected String getRealCookie(String cookieValue) {
    ...
    Stream.of(cookies).map(cookie -> cookie.split("=")).filter(cookieSplit -> cookieSplit.length == 2).forEach(cookieSplit -> {
